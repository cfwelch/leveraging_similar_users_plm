...
class Corpus(object):
    def __init__(self, pretrained_embedding, user_name, path, num_training_tokens_from_validation=2000, num_training_tokens_from_anchor=2000000, num_similar_users=10):
        ...
        with open('../../../../local2/chenxgu/present_working_directory/user-embedding-scripts-200000/validation-user-embedding-awd-lstm-lm-2000/model_{' + self.user_name + '}_{2000}.pt', 'rb') as f:
        ...
        self.num_tokens_from_anchors = (self.simis / np.sum(self.simis) * num_training_tokens_from_anchor).astype(np.int)
    ...
    def prepare_data(self, data_type):
        ...
        if data_type == 'training':
            ...
            for temp_user in self.anchor_users:
                with open(os.path.join('../../data/250000_tokens_of_anchor_users', temp_user + '_' + data_type), 'r') as file:
                    temp = file.readlines()
                    all_posts += temp

            token_ids = torch.LongTensor(counter + 200000*len(self.anchor_users))
...
if __name__ == "__main__":
    corpus = Corpus('../GloVe-1.2-emsize-200/GloVe_200', 'Jigsus', '../../data/250000_tokens_of_validation_users', num_training_tokens_from_anchor=2000000, num_similar_users=20)
