...
def main():
    ...
    for idx, user in enumerate(validation_users):
        ...
        with open(os.path.join(args.embedding_path, 'model_{' + user + '}_{' + str(args.num_training_tokens) + '}.pt'), 'rb') as f:
        ...
        for a_idx in range(len(anchor_users)):
            ...
            matrix[v_idx-100][a_idx] = np.dot(anchor_embed, valid_user_embed) / np.linalg.norm(anchor_embed) / np.linalg.norm(valid_user_embed)
