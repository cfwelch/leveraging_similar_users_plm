...
if os.path.exists(str(NUM_TOKEN) + '_tokens_of_validation_users'):
    shutil.rmtree(str(NUM_TOKEN) + '_tokens_of_validation_users')
os.makedirs(str(NUM_TOKEN) + '_tokens_of_validation_users')
...
for user in validation_users:
    print(user)
    posts = []
    with open('validation_posts_txt/' + user, 'r') as file:
        for line in file:
            posts.append(line)
    ...
    for post in posts:
        ...
        if counter + l < lengths[data_type_index]:
            ...
            counter += remaining
            with open(str(NUM_TOKEN) + '_tokens_of_validation_users/' + user + '_' + data_type[data_type_index], 'w') as file:
