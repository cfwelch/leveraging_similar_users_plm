...
def generate_confusion_matrix(model, iterator):
    confusion_matrix = torch.zeros((100, 10000), dtype=torch.int)
    model.eval()
    with torch.no_grad():
        for batch in iterator:
            ...
            for i in range(len(batch.author)):
                confusion_matrix[batch.author[i]-10000][predicted[i]] += 1
...
validation_user_dataset = TabularDataset(
                              path="../../data_10000/validation_posts_and_labels_training.csv",
                              format='csv',
                              csv_reader_params={'delimiter':'\t', 'quotechar':'"'},
                              skip_header=True,
...
with open('validation_similarity_matrix.pkl', 'wb') as file:
    pickle.dump(normalized_confusion_matrix, file)
