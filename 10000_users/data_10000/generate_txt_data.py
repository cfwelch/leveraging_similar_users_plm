...
def main():
    ...
    if os.path.exists('posts_txt'):
        shutil.rmtree('posts_txt')
    os.makedirs('posts_txt')

    n_grams = [12]

    users = []

    files = os.listdir('posts_json')

    for file in files:
        underscores = [m.start() for m in re.finditer('_', file)]
        user = file[:underscores[-3]]
        print(user)
        valid_posts = []
        invalid_posts = []
        n_grams_dict = {}
        ...
        with open('posts_json/' + user + '_json_filtered_tokenized', 'r') as file:
            ...
            for tokens in valid_posts:
                ...
                for n in n_grams:
                    for idx in range(len(tokens) - n):
                        if n_grams_dict[n][' '.join(tokens[idx: idx+n])] > 1: # is 3 in another scitps
                            repeated = True
        ...
        with open('posts_txt/' + user, 'w') as file:
            for post in final_posts:
                file.write(post + ' <eos>\n')
                # file.write(post + '\n')

        with open('posts_txt/' + user + '_invalid', 'w') as file:
