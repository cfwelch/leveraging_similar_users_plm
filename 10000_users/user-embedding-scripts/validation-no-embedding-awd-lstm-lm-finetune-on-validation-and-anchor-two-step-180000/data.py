...
class Corpus(object):
    def __init__(self, pretrained_embedding, user_name, path, num_training_tokens_from_validation, num_validation_tokens_from_validation, num_training_tokens_from_anchor, num_similar_users):
        ...
        with open("../calculate_similarity/simi_matrix_" + str(self.num_tokens_from_validation["training"]) + ".pkl", 'rb') as file:
    ...
    def prepare_data(self, data_type):
        ...
        if data_type == 'training_anchor':
            ...
            while True:
                ...
                local_counter = 0
                with open(os.path.join('/local/chenxgu/present_working_directory/data_10000/60000_tokens_of_anchor_users', current_anchor_user + '_training'), 'r') as file:
                    for post in file:
                        tokens = post.strip('\n').split(' ')
                        l = len(tokens)
...
if __name__ == "__main__":
    corpus = Corpus('../GloVe-1.2-emsize-200/GloVe_200', 'kcstrike', '/local/chenxgu/present_working_directory/data_10000/60000_tokens_of_validation_users', num_training_tokens_from_validation=2000, num_validation_tokens_from_validation=2000, num_training_tokens_from_anchor=20000, num_similar_users=20)
